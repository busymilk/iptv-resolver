...
| **`max_channels_per_title`**| `3` | `3` (推荐 3-5) | **同名电视台最大保留可用源数**。<br>- **极佳功能**：在多 IP 解析与同名电视频道测速重排序后，如果同一个电视台（如 CCTV-1）的可用源过多，系统**只保留速度最快的前 N 个**链接，**自动将速度慢的剔除抛弃**！<br>- 这能保持播放列表极致精炼，彻底避免播放器因为重名频道太多而导致死机或体验滑坡！ |
